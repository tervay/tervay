...
def score_team_event(team_number, event_key, refresh=False, wk=7):
    ...
    try:
        district_pts = call(tba.event_district_points, event=event_key,
                            refresh=refresh)
    except TypeError:
        district_pts = {'qual_points': 0, 'alliance_points': 0,
                        'elim_points': 0}
    else:
    ...
    if event_blob['event_type_string'] in ['District Championship',
    ...
    for match in matches:
        ...
        for color in ['blue', 'red']:
            if team_key in match['alliances'][color]['team_keys']:
                if match['winning_alliance'] is not None and \
                        match['winning_alliance'] == color:
                    elim_pts += 5

    to_return = [qual_pts, picking_pts, elim_pts, award_pts,
                 qual_pts + picking_pts + elim_pts + award_pts]

    if event_key in child_events.keys():
        parent = child_events[event_key]
        parent_scores = score_team_event(team_number, parent, refresh=refresh,
                                         wk=wk)
